Param(
  ...
  [string]$env:vc_redist2013u5_x86_dirname,
  [string]$env:username,
...
if( -not (Test-Path -Path "$env:tempsoft_path\$env:vc_redist2013u5_x86_dirname" ) ) { New-Item -ItemType directory -Path "$env:tempsoft_path\$env:vc_redist2013u5_x86_dirname" | Out-Null }
Copy-Item -Path "$env:softwarerepo\$env:vc_redist2013u5_x86_dirname\*" -Destination "$env:tempsoft_path\$env:vc_redist2013u5_x86_dirname\" -Force -Recurse -Confirm:$false

Write-Host "Installing Microsoft Visual C++ 2013 Update 5 (x86) Redistributable..."
$InstallExitCode = InstallMSI -MSIName "$env:vc_redist2013u5_x86_dirname" -MSIArgs "/passive /norestart"
...
Remove-Item -Path "$env:tempsoft_path\$env:vc_redist2013u5_x86_dirname" -Force -Confirm:$false -Recurse -ErrorAction SilentlyContinue
