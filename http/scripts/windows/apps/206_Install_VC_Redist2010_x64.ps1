Param(
  ...
  [string]$env:vc_redist2010sp1_x64_dirname,
  [string]$env:username,
...
if( -not (Test-Path -Path "$env:tempsoft_path\$env:vc_redist2010sp1_x64_dirname" ) ) { New-Item -ItemType directory -Path "$env:tempsoft_path\$env:vc_redist2010sp1_x64_dirname" | Out-Null }
Copy-Item -Path "$env:softwarerepo\$env:vc_redist2010sp1_x64_dirname\*" -Destination "$env:tempsoft_path\$env:vc_redist2010sp1_x64_dirname\" -Force -Recurse -Confirm:$false

Write-Host "Installing Microsoft Visual C++ 2010 SP1 (x64) Redistributable..."
$InstallExitCode = InstallMSI -MSIName "$env:vc_redist2010sp1_x64_dirname" -MSIArgs "/passive /norestart"
...
Remove-Item -Path "$env:tempsoft_path\$env:vc_redist2010sp1_x64_dirname" -Force -Confirm:$false -Recurse -ErrorAction SilentlyContinue
